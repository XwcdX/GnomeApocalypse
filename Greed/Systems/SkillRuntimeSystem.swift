...
/// Runs active skill effects that need frame-based state, rendering, and collision checks.
final class SkillRuntimeSystem {
    ...
    private let particleAssets: ParticleAssets
    ...
    init(audioManager: AudioManager = .shared, particleAssets: ParticleAssets = .shared) {
        self.audioManager = audioManager
        self.particleAssets = particleAssets
    }

    /// Advances all active skill runtimes for the supplied gameplay scene state.
    func update(
        ...
        updateWardenThorns(deltaTime: deltaTime, players: players, enemies: enemies, scene: scene)
    }

    private func updateLightningSkills(
        deltaTime: TimeInterval,
        players: [PlayerEntity],
        enemies: [EnemyEntity],
        scene: SKNode,
        ...
        for player in players {
            ...
            guard castLightning(for: player, enemies: enemies, scene: scene, cameraSystem: cameraSystem) else { continue }
    ...
    private func castLightning(
        ...
        let aliveEnemies = enemies.filter { $0.parent != nil && isInsideCamera($0.position, inset: inset, cameraSystem: cameraSystem) }
        ...
        for _ in 0..<strikeCount {
            ...
            strikeLightning(on: target, damage: SkillConfig.lightningBaseDamage, scene: scene)
        }

        return true
    }

    private func strikeLightning(on enemy: EnemyEntity, damage: Int, scene: SKNode) {
        let position = enemy.position
        let strike = makeLightningStrikeNode(at: position, radius: referenceSpriteHeight * lightningStrikeRadiusFactor)
        scene.addChild(strike)
        particleAssets.emit(.lightningImpact, at: position, in: scene)
        enemy.takeDamage(damage)
    }

    private func updateMistSkills(
        deltaTime: TimeInterval,
        players: [PlayerEntity],
        enemies: [EnemyEntity],
        scene: SKNode,
        ...
        for player in players {
            ...
            tickMistClouds(for: playerID, deltaTime: deltaTime, enemies: enemies)
            ...
            if activeCount < player.mistCloudCount, cooldown == 0 {
                spawnMistCloud(for: player, scene: scene, cameraSystem: cameraSystem)
                mistSpawnCooldowns[playerID] = player.mistCooldown
            }
        }
    }

    private func tickMistClouds(for playerID: ObjectIdentifier, deltaTime: TimeInterval, enemies: [EnemyEntity]) {
        ...
        for index in clouds.indices {
            ...
            while clouds[index].tickAccumulator >= SkillConfig.mistTickInterval {
                ...
                applyMistDamage(from: clouds[index], enemies: enemies)
            }
        }
    ...
    private func spawnMistCloud(for player: PlayerEntity, scene: SKNode, cameraSystem: CameraSystem) {
        let playerID = ObjectIdentifier(player)
        let position = randomPointInCameraView(cameraSystem: cameraSystem)
        ...
        audioManager.play(.mistExplosion)
        particleAssets.emit(.mistExplosion, at: position, in: scene)
    }

    private func applyMistDamage(from cloud: ActiveMistCloud, enemies: [EnemyEntity]) {
    ...
    private func updateWardenThorns(
        deltaTime: TimeInterval,
        players: [PlayerEntity],
        enemies: [EnemyEntity],
        scene: SKNode
        ...
        for player in players {
            ...
            reconcileThornCount(for: playerID, desired: desired, scene: scene)
            advanceThornAngles(for: playerID, deltaTime: deltaTime)
            updateThornPositions(for: player)
            decayWardenThornCooldowns(for: playerID, deltaTime: deltaTime, enemies: enemies)
            applyThornCollisions(for: player, enemies: enemies, scene: scene)
        }
    }

    private func reconcileThornCount(for playerID: ObjectIdentifier, desired: Int, scene: SKNode) {
    ...
    private func decayWardenThornCooldowns(
        for playerID: ObjectIdentifier,
        deltaTime: TimeInterval,
        enemies: [EnemyEntity]
    ) {
    ...
    private func applyThornCollisions(for player: PlayerEntity, enemies: [EnemyEntity], scene: SKNode) {
        let playerID = ObjectIdentifier(player)
        guard let thorns = wardenThorns[playerID] else { return }
        ...
        for (thornIndex, thorn) in thorns.enumerated() {
            ...
            for enemy in enemies where enemy.parent != nil {
                ...
                particleAssets.emit(.wardenThornsHit, at: enemy.position, in: scene)
    ...
    private func randomPointInCameraView(cameraSystem: CameraSystem) -> CGPoint {
    ...
    private func isInsideCamera(_ position: CGPoint, inset: CGFloat, cameraSystem: CameraSystem) -> Bool {
